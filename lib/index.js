...
async function readToday(origin, apiKey, units) {
  const from = Math.floor(localDayStartMs() / 1e3);
  const to = Math.floor(Date.now() / 1e3) + 60;
  const tryStat = async () => {
...
function listRouteAccounts(ctx) {
  ...
  for (const entry of llm.listConfigurableProviders()) {
    ...
    const origin = originOf(profile?.baseURL ?? profile?.baseUrl);
...
async function fetchWallet(ctx) {
  const account = currentAccount(ctx);
  if (account === void 0) return { ok: false, error: "no-provider" };
